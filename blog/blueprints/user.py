...
from ..model import Comment, comments_likes, Notification, User
...
def get_user(id):
    ...
    if g.current_user == que:
        return jsonify(que.to_dict())    # 查询自己的信息
    data = que.to_dict()
    # 如果是查询其它用户，添加 是否已关注过该用户 的标志位
    data['is_following'] = g.current_user.is_following(que)
    return jsonify(data)
...
def follow(id):
    ...
    # 更新未读通知数目
    que.add_new_notification('new_received_followers', que.new_received_followers())
    db.session.commit()
    return 'Success'
...
def get_ones_fans(id):
    ...
    per_page = 4
    # que.followers得到que的所有粉丝，分页
    pagi = User.paginated_dict(que.followers, page, per_page, 'user.get_ones_fans', id=id)
    for item in pagi['items']:
        # 对于que的每个粉丝item['id']，查看g.current_user是否关注过
        item['is_following'] = g.current_user.is_following(User.query.get(item['id']))
        res = db.engine.execute("select * from followers where follower_id={} and followed_id={}"
                                .format(item['id'], que.id))   # 获得关联表中timestamp
...
def get_ones_followeds(id):
    ...
    per_page = 4
    # que.followers得到que的所有粉丝，分页
    pagi = User.paginated_dict(que.followeds, page, per_page, 'user.get_ones_followeds', id=id)
    for item in pagi['items']:
        # 对于que的每个粉丝item['id']，查看g.current_user是否关注过
        item['is_following'] = g.current_user.is_following(User.query.get(item['id']))
    return jsonify(pagi)
...
def get_received_comments(id):
    ...
    pagi = Comment.paginated_dict(
        # 得到用户所有文章的所有他人评论
        Comment.query.filter(Comment.article_id.in_(post_ids), Comment.author != g.current_user).
...
def get_followed_posts(id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = 5
    pagi = User.paginated_dict(que.followed_posts(), page, per_page,
                               'user.get_followed_posts', id=id)
    return jsonify(pagi)
